...
sub vereval
{
 ...
 for (my $i=0;$i<=$#CURRENT;$i++)
  {
   if (@CURRENT[$i] > @RUNNING[$i]) { return 1; }  # running is older
   if (@CURRENT[$i] < @RUNNING[$i]) { return 0; }  # running is newer
   if ((@CURRENT[$i] ne "") && (@RUNNING[$i] eq "")) { return 1; }  # running is older
  }

 return 0;  # running is OK
}
