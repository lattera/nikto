...
Thanks:		Rain Forest Puppy (Whisker, LibWhisker)
       		Paul Darby (ongoing testing, comments, and bad music)
       		Steve Valdez (ongoing testing, comments, adding himself to the Admin group)
       		Steve Saady (testing, comments)
       		Prickley Paw for a large chunk of scan items (1/16/2002)
...
### ----------------------------------------------------------------------------------------- ###
6.  Known Issues
- CLI Options cannot be combined (i.e., -dv)_
- Proxy set up (and more?) must move to config file
- Safety check of the plugins
- Some issues with finding the plugins directory if nikto.pl is not in current dir
- Add CGIDIRS to one of the config files
...
1.016	01.10.2002	Add dump_*hash functions
...
1.017	01.12.2002	Fixed a bug where the data portion of a request did not reset to null after some checks (thanks to Phil Brass for pointing me at it & letting me test against his server). 
1.018	01.17.2002	Added /mpcgi/ to the @CGIDIRS array based on some suggestions.
