...
use vars qw/%REALMS %REALMS_TESTED %CONFIG %NIKTO %OUTPUT %METHD %RESPS %INFOS %SERVER %request %result %JAR %DATAS %COUNTERS/;
...
set_targets();
load_scan_items();
%REALMS=load_realms("$NIKTO{plugindir}/realms.db");
